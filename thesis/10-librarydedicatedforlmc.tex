...
\subsection{LMR files}\label{lmrFilesSubsection}
This is a file format specially developed for the LeapGesture library, supported by various modules for example by the visualizer described in Section~\ref{recordvisualSection}. The file structure is as follows:
\begin{itemize}
...
Then the library executes preprocessing on the given training set and starts training by using the selected module by API user:
\begin{itemize}
