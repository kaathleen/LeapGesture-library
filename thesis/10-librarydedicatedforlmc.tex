...
Each of those modules has its own separate methods -- for learning new gestures, and for further operational work (recognition). They are implemented using the observer pattern. For each of the modules, there are specified relevant events that are fired in the key moments of recognition such as: moment in which particular gesture began to be recognized, moment in which the particular gesture stopped being recognized, moment in which frame processing is finished.
...
After the start of recognition process LeapGesture library notifies the object of \texttt{RecognizedGestureListener} regards the occurrence of gestures recognition events.
